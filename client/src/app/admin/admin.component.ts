...
export class AdminComponent implements OnInit {
  ...
  constructor(public donor: DonorService, private router: Router,public receiverService:ReceiverService,private fb:FormBuilder) {
 ...
 getalldonors()
  ...
  getallreceivers()
  {
    if(!this.showreceivers)//if false
    {
      ...
      this.receiverService.getReceivers().subscribe((res) => {
  ...
  deleteReceiver(receiverdata) {
    ...
    this.receiverService.deleteReceiver(receiverdata).subscribe((res) => {
      if (res == true) {
        console.log('deleted');
